...
chat. But that is not a licence to guess: see rule 8.
...
5. **PERFORM the task's `done when:` check literally — do not approximate it.** The
   acceptance clause names the real check; satisfy THAT, not a cheaper proxy:
   - "observed via `bin/dev`" / "renders" / "clicking X does Y" → actually run the
     app and drive it. You have a shell: start the server, render the page in headless
     Chromium (`chromium --headless`, a Playwright/Puppeteer script, or `curl` for
     endpoints), assert on the **computed/rendered** result and post-interaction DOM —
     NOT on whether an HTML string contains some markup. A unit test over markup passes
     on exactly the CSS/interaction bugs that break the live page; it is not the check.
